...
abstract class baformsHelper 
{
    ...
    public static function checkIntegration($type)
    {
        switch ($type) {
            case 'google_sheets':
                $path = JPATH_ROOT.'/components/com_baforms/libraries/google-sheets/baSheets.php';
                break;
            case 'google_drive':
            ...
            case 'campaign_monitor':
                $path = JPATH_ROOT.'/components/com_baforms/libraries/campaign-monitor/campaign.php';
                break;
            case 'zoho_crm':
                $path = JPATH_ROOT.'/components/com_baforms/libraries/zoho-crm/zoho.php';
                break;
        }
        $className = !JFile::exists($path) ? ' require-library' : '';

        return $className;
    }

    public static function getGoogleAuth($scope)
    {
        $redirect = 'urn:ietf:wg:oauth:2.0:oob';
        $clientId = '438976022629-rmthiukas4ln5em6t984mv7tvo2dn6mh.apps.googleusercontent.com';
        $scopes = implode(' ', $scope);
        $auth = 'https://accounts.google.com/o/oauth2/auth?response_type=code&redirect_uri=';
        $auth .= urlencode($redirect).'&client_id='.urlencode($clientId).'&scope=';
        $auth .= urlencode($scopes).'&access_type=offline&approval_prompt=auto';
    ...
    public static function getIntegrations($id)
    {
        ...
        $array = array('google_drive','google_maps', 'google_sheets', 'paypal', 'twocheckout', 'mailchimp', 'mollie',
            ...
            'getresponse', 'zoho_crm', 'robokassa', 'flutterwave');
        $where = array();
        ...
        if (empty($data->google_drive)) {
            $data->google_drive = self::addIntegration('google_drive', '{"code":"", "accessToken":""}');
        }
