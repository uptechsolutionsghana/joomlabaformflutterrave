...
function columnResizer(event) {
	var leftEl = $f(this).prev(),
		...
		padding =
			style.paddingRight.replace('px', '') * 1 +
			style.paddingLeft.replace('px', '') * 1,
		lpadding =
			lstyle.paddingRight.replace('px', '') * 1 +
			lstyle.paddingLeft.replace('px', '') * 1,
	...
	padding +=
		style.borderLeftWidth.replace('px', '') * 1 +
		style.borderRightWidth.replace('px', '') * 1
	lpadding +=
		lstyle.borderLeftWidth.replace('px', '') * 1 +
		lstyle.borderRightWidth.replace('px', '') * 1
	$f(document)
		.on('mousemove.resize', function (event) {
...
function pageResizer(event) {
	var page = $f(this).closest('.ba-form-page'),
		info = page
			.find('> .page-info')
			.addClass('visible-info ' + this.dataset.position),
	...
	$f(document)
		.on('mousemove.resize', function (event) {
			...
			$f(
				'input[type="number"][data-subgroup="width"][data-option="value"]'
			).val(value)
...
function setMinicolorsColor(value) {
	...
	$f('.variables-color-picker')
		.minicolors('value', obj)
		.closest('#color-picker-cell')
		.find('.minicolors-opacity')
		.val(color[1])
	$f('#color-variables-dialog .active').removeClass('active')
	$f(
		'#color-picker-cell, #color-variables-dialog .nav-tabs li:first-child'
	).addClass('active')
}
...
function returnPointLibraryItem(event) {
	var pageY = event.clientY,
		...
		columns = [].slice.call(
			document.querySelectorAll('.ba-form-column-wrapper > .ba-form-column')
		)
	columns = columns.reverse()
	for (var i = 0; i < columns.length; i++) {
		$f(columns[i])
			.find(' > .ba-form-field-item')
			.each(function () {
				...
				if (
					rect.top - marginTop < event.clientY &&
					rect.bottom + marginBottom > event.clientY &&
...
function checkIframe(modal, view, callback) {
	...
	if (iframe.attr('src').indexOf('view=' + view) == -1) {
		iframe[0].src =
			'index.php?option=com_baforms&view=' + view + '&tmpl=component'
...
function prepareFields(key, field) {
	...
	if (field) {
		...
		if ('title' in app.items[id] && key != 'acceptance') {
			...
			content.querySelector('.ba-input-label-wrapper').textContent =
				formsLanguage['ENTER_FIELD_TITLE']
		}
		if ('confirm' in app.items[id]) {
			app.items[id].confirm.title = formsLanguage['CONFIRM_EMAIL']
			app.items[id]['confirm-password'].title =
				formsLanguage['CONFIRM_PASSWORD']
		}
	}
	...
	app = {
		...
		getTextContent: function (textarea) {
			...
			document.querySelector(
				'#' + app.selector + ' .text-content-wrapper'
			).innerHTML = data
			app.edit.html = data
			app.buttonsPrevent(
				$f('.ba-forms-workspace').find('a, input[type="submit"], button')
			)
		}
	},
	mailchimp = {},
...
app.loadGoogleMaps = function (search) {
	if (!app.googleMaps) {
		...
		$f(search).each(function () {
			app.createGoogleMap(
				this.querySelector('.ba-map-wrapper'),
				app.items[this.id]
			)
		})
	}
...
app.createLocationMap = function () {
	...
	if (app.googleMaps) {
		...
		app.locationMap = new google.maps.Map(
			document.querySelector('.ba-address-map-wrapper'),
			options
		)
...
!(function (d) {
	function getTemplatesElement(obj) {
		...
		script.src =
			'https://www.balbooa.com/updates/baforms/formsApi/' + obj.key + '.js'
		script.onload = function () {
			app.formsTemplates.push(formsApi.templates[obj.group][obj.key])
			el.querySelector('.templates-element-image').style.backgroundImage =
				'url(' + formsApi.templates[obj.group][obj.key].imageData + ')'
		}
		...
		script.onload = function () {
			if (formsApi.templates) {
				for (let ind in formsApi.templates) {
					if (
						!d.querySelector('#templates-modal li[data-group="' + ind + '"]')
					) {
					...
					for (let key in formsApi.templates[ind]) {
						if (
							d.querySelector(
								'#templates-modal .templates-element[data-group="' +
								ind +
								'"][data-key="' +
								key +
								'"]'
	...
	if (app.edit.marker.position) {
		...
		if (app.edit.marker.description) {
			...
			if (app.edit.marker.infobox) {
				app.locationMap.marker.infoWindow.open(
					app.locationMap,
					app.locationMap.marker
				)
			}
...
app.ckeLink = function () {
	$f('.post-link-input')
		...
		.on('blur', function () {
			this.closest('.link-picker-container').classList.remove(
				'focus-link-input'
			)
		})
...
app.customSelect = function () {
	$f('.ba-custom-select > i, div.ba-custom-select input').on('click', function (
		event
	) {
		event.stopPropagation()
		var $this = $f(this),
		...
		this.delay = setTimeout(function () {
			if (!search) {
				...
				li.each(function () {
					...
					if (
						font.indexOf(search) < 0 ||
						this.classList.contains('popular-fonts')
					) {
...
app.setTooltip = function (parent) {
	parent
		...
		.on('mouseenter', function () {
			if (
				this.classList.contains('ba-form-field') &&
				!this.classList.contains('disabled-field-drag') &&
...
app.loadCodeMirror = function (callback) {
	...
	file.onload = function () {
		...
		file.type = 'text/javascript'
		file.src =
			JUri + 'media/editors/codemirror/addon/hint/javascript-hint.min.js'
		document.head.appendChild(file)
		file = document.createElement('script')
		file.type = 'text/javascript'
		...
		file.type = 'text/javascript'
		file.src =
			JUri + 'media/editors/codemirror/mode/javascript/javascript.min.js'
		...
		file.onload = function () {
			...
			file.type = 'text/javascript'
			file.src =
				JUri + 'media/editors/codemirror/mode/htmlmixed/htmlmixed.min.js'
			document.head.appendChild(file)
			file.onload = function () {
				...
				app.HTMLEditor.on('change', function (from, too) {
					...
					this.delay = setTimeout(function () {
						var value = app.HTMLEditor.getValue(),
							item = document
								.getElementById(app.selector)
								.querySelector('div.custom-html-wrapper')
...
app.loadDraggable = function () {
	...
	file.onload = function () {
		$f(
			'.code-editor-dialog, .draggable-modal-cp, #text-editor-dialog'
		).draggable({
			handle: '.modal-header'
		})
	}
...
app.setMediaManager = function () {
	...
	$f('.show-media-manager').on('mousedown', function () {
		setTimeout(function () {
			uploadMode = null
			checkIframe(
				$f('#uploader-modal').attr('data-check', 'multiple'),
				'uploader',
				function () {
					var iframe = document.querySelector('#uploader-modal iframe')
						.contentWindow
...
app.setAddFields = function () {
	$f('.ba-form-fields-list').on('mousedown', '.ba-form-field', function (
		event
	) {
		...
		$f(document)
			.on('mousemove.library', function (event) {
				libHandle.style.top = event.clientY + 'px'
				libHandle.style.left = event.clientX + 'px'
				...
				if (item) {
					...
					next =
						...
						(rect.bottom + marginBottom - (rect.top - marginTop)) >
						0.5
					var obj = {
						left: rect.left + 16,
			...
			.on('mouseup.library', function () {
				...
				if (item) {
					...
					if (key == 'image') {
						...
						uploadMode = 'addImage'
						checkIframe(
							$f('#uploader-modal').attr('data-check', 'single'),
							'uploader'
						)
					} else {
						app.dropField(data)
...
app.addLibrary = function (type, passive) {
	if (
		type == 'google_drive' &&
		$f('.integrations-element[data-type="pdf_submissions"]').hasClass(
			'require-library'
		)
	) {
		app.addLibrary('pdf_submissions', true)
	}
	...
	$f.ajax({
		...
		success: function (msg) {
			$f('.integrations-element[data-type="' + type + '"]').removeClass(
				'require-library'
			)
			if (!passive) {
				showNotice(msg)
...
app.requireIntegrationLibrary = function (type) {
	if (app.loadingLibrary[type]) {
		return false
	}
	app.loadingLibrary[type] = true
	var installing =
		formsLanguage['INSTALLING'] +
		'<img src="' +
		JUri +
		'components/com_baforms/assets/images/reload.svg"></img>',
		script = document.createElement('script')
	notification[0].className = 'notification-in'
	notification.find('p').html(installing)
	script.src =
		'https://www.balbooa.com/updates/baforms/formsApi/integrations/' +
		formsApi.integrations[type].script
	script.onload = function () {
		app.addLibrary(type)
	}
...
app.getSortingImagesHTML = function (i, obj) {
	...
	div.innerHTML =
		'<label class="ba-form-checkbox"><input type="checkbox"><span></span></label><div class="images-sortable-handle"></div>'
		...
		case 'checkboxImage':
			fontBtn.rows.each(function (i) {
				if (fontBtn.keys.indexOf(i) != -1) {
					...
					img.innerHTML =
						'<i class="zmdi zmdi-delete delete-sorting-image"></i>'
					wrapper.innerHTML = ''
...
app.setMinicolors = function () {
	$f('input[data-type="color"]')
		...
		.on('click', function () {
			...
			$f('#color-variables-dialog')
				.css({
					left: rect.left - 285,
					top: rect.bottom - (rect.bottom - rect.top) / 2 - 174
				})
				.removeClass('ba-right-position ba-bottom-position ba-top-position')
				.modal()
		})
				...
				if (callback) {
					...
					if (
						...
						$f(
							'[data-group="' +
							group +
							'"][data-subgroup="background"][data-option="color"]'
						)
							.not($this)
							.each(function () {
						...
						let str =
							'[data-group="label"][data-subgroup="typography"][data-option="color"],' +
	...
	$f('#color-variables-dialog .minicolors-opacity').on('input', function () {
		...
		fontBtn.dataset.rgba = $f('.variables-color-picker').minicolors(
			'rgbaString'
		)
		...
		if (fontBtn.localName == 'input') {
			$f(fontBtn)
				.next()
				.find('.minicolors-swatch-color')
				.css('background-color', fontBtn.dataset.rgba)
				.closest('.minicolors')
				.next()
				.find('.minicolors-opacity')
				.val(this.value)
...
function showLightboxTriggerOptions() {
	...
	if (app.edit.trigger.type != '') {
		modal
			.find('.lightbox-session-options.lightbox-trigger-options')
			.css('display', '')
	}
	modal
		.find(
			'.lightbox-trigger-options[data-trigger="' + app.edit.trigger.type + '"]'
		)
		.css('display', '')
...
function showLightboxSessionOptions() {
	$f('.lightbox-session-duration').css(
		'display',
		app.edit.session.enable ? '' : 'none'
	)
}
...
app.showLightboxSettings = function () {
	...
	modal
		.find(
			'.lightbox-position-wrapper div[data-value="' + app.edit.position + '"]'
		)
		.addClass('active')
...
app.checkboxFieldAction = function (value, group, subgroup, option) {
	if (option == 'count') {
		let item = document.querySelector(
			'#' + app.selector + ' .ba-form-checkbox-group-wrapper'
		)
...
app.mapFieldAction = function (value, group, subgroup, option) {
	...
	if (
		...
		app.createGoogleMap(
			document.querySelector('#' + app.selector + ' .ba-map-wrapper'),
			app.edit
		)
	}
}
...
app.calendarFieldAction = function (value, group, subgroup, option) {
	...
	if (option == 'title') {
		...
		$f(item)
			.find('.ba-field-container input')
			.attr('placeholder', value)
	} else if (option == 'description' && value) {
		let tooltip = item.querySelector(
			'#' +
			app.selector +
			' > .ba-input-wrapper .ba-field-label-wrapper .ba-tooltip'
		)
		if (!tooltip) {
			let span = document.createElement('span')
			span.className = 'ba-input-help'
			span.innerHTML =
				'<i class="zmdi zmdi-help"></i><span class="ba-tooltip ba-top ba-hide-element"></span>'
			tooltip = span.querySelector('.ba-tooltip')
			$f(item)
				.find('> .ba-input-wrapper .ba-field-label-wrapper')
				.append(span)
		...
		$f(item)
			.find('> .ba-input-wrapper .ba-field-label-wrapper .ba-input-help')
			.remove()
		...
		item
			.querySelector('.ba-field-container')
			.classList.add('ba-readonly-calendar')
	} else if (option == 'readonly' && !value) {
		item
			.querySelector('.ba-field-container')
			.classList.remove('ba-readonly-calendar')
	} else if (option == 'type') {
		item
			.querySelector('.ba-field-container')
			.classList[value ? 'add' : 'remove']('calendar-range-type')
	}
}
...
app.uploadFieldAction = function (value, group, subgroup, option) {
	...
	if (option == 'title') {
		...
		item
			...
			.classList[value ? 'add' : 'remove']('drag-drop-upload-file')
	}
}
...
app.prepareShortCodes = function () {
	...
	formShortCodes['[Form Title]'] = document.querySelector(
		'.ba-form-title'
	).value
	...
	$f('.ba-form-title').on('input', function () {
		...
		this.delay = setTimeout(function () {
			...
			for (let ind in app.items) {
				...
				if (
					item &&
					(item.dataset.type == 'input' || item.dataset.type == 'address')
				) {
					value = renderDefaultValue(app.items[ind].default)
					item.querySelector(
						'.ba-field-container input, .ba-field-container textarea'
					).value = value
				}
			}
		}, 500)
...
app.appendConditionRow = function (i, key) {
	let div = templates['condition-logic-' + key].content.cloneNode(true),
		...
		str = ''
	div.querySelector(
		'.condition-logic-horizontal-fields-wrapper'
	).dataset.key = i
	...
	if (key == 'when') {
		app.checkConditionFieldType(
			div.querySelector('.condition-logic-horizontal-fields-wrapper'),
			app.edit[key][i]
		)
	} else {
		app.checkConditionDoAction(
			div.querySelector('.condition-logic-horizontal-fields-wrapper'),
			app.edit[key][i]
		)
	}
	div
		.querySelectorAll('select[data-key], input[data-key]')
		.forEach(function (el) {
	...
	$f(
		'#condition-logic-modal .condition-logic-' +
		key +
		'-group .add-new-select-item-wrapper'
	).before(div)
}
...
app.checkConditionDoAction = function (div, obj) {
	...
	if (obj.action == 'move') {
		...
		content
			.querySelector('.forms-fields-list')
...
app.checkConditionFieldType = function (div, obj) {
	...
	state
		.find(
			'option[value="greater"], option[value="less"], option[value="contain"], option[value="not-contain"]'
		)
		.each(function () {
			let flag =
				item &&
				(app.items['baform-' + obj.field].items ||
					item.dataset.type == 'acceptance' ||
					item.dataset.type == 'address')
...
app.sliderFieldAction = function (value, group, subgroup, option) {
	...
	if (option == 'type' && value == 'range') {
		let str =
			'<div class="form-range-wrapper"><span class="ba-form-range-liner"></span>' +
			...
			'<input type="text" data-type="min" class="set-slider-range" value="' +
			app.edit.min +
			'">' +
			...
			app.edit.step +
			'" data-type="range">' +
			'<input type="text" data-type="max" class="set-slider-range" value="' +
			app.edit.max +
			'">' +
			'</div>'
		...
		let str =
			'<div class="form-slider-wrapper"><span class="ba-form-range-liner" style="width: 100%;"></span>' +
			'<input type="range" min="' +
			app.edit.min +
			'" max="' +
			app.edit.max +
			...
			'" value="' +
			app.edit.min +
			'" data-index="0"><input type="range" min="' +
			app.edit.min +
			'" max="' +
			app.edit.max +
			'" step="' +
			...
			app.edit.max +
			'" data-index="1"></div>' +
			'<div class="form-slider-input-wrapper"><input type="number" data-type="slider" step="' +
			app.edit.step +
			'" data-index="0" value="' +
			app.edit.min +
			'"><input type="number" data-type="slider" step="' +
			app.edit.step +
			'" data-index="1" value="' +
			app.edit.max +
		...
		$f(item)
			...
			.trigger('input')
		$f(item)
			.find('.form-slider-wrapper input[type="range"]')
			.last()
			.val(app.edit.max)
			.trigger('input')
		$f(item)
			.find('.set-slider-range[data-type="min"]')
			.val(value)
	} else if (option == 'max') {
		$f(item)
			.find('input[type="range"]')
		...
		$f(item)
			...
			.trigger('input')
		$f(item)
			.find('.set-slider-range[data-type="max"]')
			.val(value)
	} else if (option == 'step') {
		$f(item)
			.find('input')
...
app.calculationAction = function (value, group, subgroup, option) {
	...
	if (option == 'design' && !value) {
		...
		item
			.find('.ba-form-products-cart')
		[value ? 'removeClass' : 'addClass']('disabled-cart-products')
	} else if (option == 'position') {
		item
			.find('.ba-field-container, .ba-form-products-cart')
		[value ? 'addClass' : 'removeClass']('right-currency-position')
	} else if (
		option == 'thousand' ||
		option == 'separator' ||
		option == 'decimals'
		...
		if (item[0].dataset.type == 'calculation') {
			...
			item
				.find('.ba-cart-subtotal-row, .ba-form-product-total-cell')
				.find('.field-price-value')
				.text(price)
...
app.calculateCartTotal = function (id, obj) {
	...
	item.find('.ba-cart-shipping-item input[type="radio"]').each(function () {
		shippingFlag = true
		price = renderPrice(
			String(this.dataset.price),
			obj.thousand,
			obj.separator,
			obj.decimals
		)
		this.closest('.ba-cart-shipping-item').querySelector(
			'.field-price-value'
		).textContent = price
		if (this.checked) {
...
app.navigationAction = function (value, group, subgroup, option) {
	...
	if (option == 'style') {
		$f('#navigation-settings-dialog select[data-option="style"] option').each(
			function () {
...
app.loadRecaptcha = function () {
	...
	script.src =
		'https://www.google.com/recaptcha/api.js?onload=formsRecaptchaOnload&render=explicit'
...
app.acceptanceFieldAction = function (value, group, subgroup, option) {
	if (option == 'html') {
		...
		document.querySelector(
			'#' + app.selector + ' .ba-form-acceptance-html'
		).innerHTML = value
		app.buttonsPrevent(
			$f('.ba-forms-workspace').find('a, input[type="submit"], button')
		)
...
app.submitFieldAction = function (value, group, subgroup, option) {
	...
	if (keys.indexOf(group) != -1) {
		...
		if (
			(group == 'padding' && (option == 'top' || option == 'bottom')) ||
			(group == 'typography' && option == 'line-height')
		) {
		...
		if (!i) {
			...
			$f(item)
				.find('.ba-form-submit-btn')
				.prepend(i)
		}
		i.className = value
	} else if (option == 'submit-icon' && !value) {
		$f(item)
			.find('.ba-form-submit-btn i')
			.remove()
	} else if (
		option == 'onclick' ||
		((group == 'reply' || group == 'notifications') && option == 'enable')
	) {
		...
		document
			.querySelectorAll(
				'#submit-button-settings-dialog select[data-option="animation"] option'
			)
			.forEach(function (el) {
...
app.ratingFieldAction = function (value, group, subgroup, option) {
	let item = document.querySelector('#' + app.selector)
	if (option == 'layout') {
		let classList = item.querySelector('.ba-form-rating-group-wrapper')
			.classList
...
app.inputFieldAction = function (value, group, subgroup, option) {
	...
	if (option == 'title') {
		...
		$f(item)
			.find(query)
			.find(' > .ba-field-container')
			.find('input, textarea')
			.attr('placeholder', value)
	} else if (option == 'icon' && value) {
		let i = item.querySelector(
			'#' + app.selector + ' > ' + query + ' .ba-field-container > i'
		)
		if (!i) {
			i = document.createElement('i')
			$f(item)
				.find('> ' + query + ' .ba-field-container')
				.prepend(i)
		}
		i.className = value
	} else if (option == 'icon' && !value) {
		$f(item)
			.find('> ' + query + ' .ba-field-container > i')
			.remove()
		...
		let tooltip = item.querySelector(
			...
			' > ' +
			query +
			' .ba-field-label-wrapper .ba-tooltip'
		)
		if (!tooltip) {
			let span = document.createElement('span')
			span.className = 'ba-input-help'
			...
			tooltip = span.querySelector('.ba-tooltip')
			$f(item)
				.find('> ' + query + ' .ba-field-label-wrapper')
				.append(span)
			app.setTooltip($f(span))
		}
		tooltip.textContent = value
		...
		$f(item)
			.find('> ' + query + ' .ba-field-label-wrapper .ba-input-help')
			.remove()
	} else if (option == 'default') {
		value = renderDefaultValue(value)
		$f(item)
			...
			.val(value)
		$f(item)
			.find('.ba-field-container .current-characters')
			.text(value.length)
		...
		$f(item)
			.find('.ba-input-wrapper .ba-field-container')
			.each(function () {
				let input = this.querySelector('input, textarea'),
					element = document.createElement(
						value == 'textarea' ? 'textarea' : 'input'
					),
		...
		let type = value == 'numbers' ? 'number' : 'text'
		$f(item)
			.find(query)
			.find(' > .ba-field-container input')
			.attr('type', type)
...
function setInputCharacters() {
	$f('#' + app.selector)
		.find('.ba-field-container .characters-wrapper')
		.remove()
	if (
		app.edit.characters.length != '' &&
		(app.edit.type == 'text' || app.edit.type == 'textarea')
	) {
		let key = app.edit.characters.key.toUpperCase(),
			substr =
				...
				app.edit.characters.length +
				' ' +
				formsLanguage['CHARACTERS'] +
				')',
			length = app.edit.default.length,
			str =
				'<span class="characters-wrapper"><span class="current-characters">' +
				length +
				'</span><span class="limit-characters">' +
				substr +
				'</span></span>'
		$f('#' + app.selector)
			.find('.ba-field-container')
...
function prepareConfirm() {
	...
	if (app.edit.type == 'email' && app.edit.confirm.enable) {
		...
		for (let ind in app.edit.confirm) {
			...
			$f(
				'#input-field-settings-dialog input[data-group="confirm"][data-option="' +
				ind +
				'"]'
			).trigger('input')
		}
...
function preparePasswordConfirm() {
	...
	if (app.edit.type == 'password' && app.edit['confirm-password'].enable) {
		...
		clone
			.find('.ba-input-label-wrapper')
			.text(app.edit['confirm-password'].title)
		clone
		...
		for (let ind in app.edit['confirm-password']) {
			...
			$f(
				'#input-field-settings-dialog input[data-group="confirm-password"][data-option="' +
				ind +
				'"]'
			).trigger('input')
		}
...
function prepareRequired() {
	$f('#' + app.selector)
		.find('.ba-input-wrapper, .confirm-password-wrapper')
		.find('span.required-star')
		.remove()
...
function prepareSubmitOptions() {
	$f('#submit-button-settings-dialog')
		.find('[class*="-options"]')
		.hide()
	$f('#submit-button-settings-dialog')
		.find('.' + app.edit.onclick + '-options')
		.css('display', '')
	if (app.edit.notifications.enable) {
		$f('input[data-group="notifications"][data-option="enable"]')
			.closest('.ba-settings-item')
			.nextAll()
			.css('display', '')
	} else {
	...
	if (app.edit.reply.enable) {
		$f('input[data-group="reply"][data-option="enable"]')
			.closest('.ba-settings-item')
			.nextAll()
			.css('display', '')
	} else {
...
function prepareInputType() {
	...
	modal
		.find(
			'.email-type-options, .not-email-type-options, .not-mask-type, .mask-type-options, .password-type-options'
		)
		.hide()
	modal
		.find('.not-email-type-options.ba-settings-group')
		.first()
		.prev()
		.addClass('last-visible-group')
	if (app.edit.type == 'email') {
		modal
			.find('.email-type-options, .not-mask-type')
			.not('.not-email-type-options')
			.css('display', '')
		modal
			.find('.email-confirm-options')
			.css('display', app.edit.confirm.enable ? '' : 'none')
		modal
			.find('input[data-option="required"]')
			.attr('disabled', 'disabled')
			.prop('checked', true)
		changeSelected(
			modal
				.find('select[data-option="validation"]')
				.attr('disabled', 'disabled'),
			'email'
		)
	} else if (app.masks.indexOf(app.edit.type) != -1) {
		modal
			.find('.mask-type-options input[data-option="mask"]')
			.val(app.edit.mask)
		modal.find('.mask-type-options').css('display', '')
		modal.find('input[data-option="required"]').removeAttr('disabled')
		changeSelected(
			modal.find('select[data-option="validation"]').removeAttr('disabled'),
			''
		)
	} else if (app.edit.type == 'password') {
		modal.find('.password-type-options, .not-mask-type').css('display', '')
		modal
			.find('.password-confirm-options')
			.css('display', app.edit['confirm-password'].enable ? '' : 'none')
		modal.find('input[data-option="required"]').removeAttr('disabled')
	} else {
		...
		modal
			...
			.first()
			.prev()
			.removeClass('last-visible-group')
		modal.find('input[data-option="required"]').removeAttr('disabled')
		changeSelected(
			modal.find('select[data-option="validation"]').removeAttr('disabled'),
			''
...
function getSortingItemHTML(obj, ind) {
	let div = document.createElement('div'),
		...
		str =
			'<div class="sorting-icon sortable-handle"><i class="zmdi zmdi-more-vert"></i></div>',
		focus = (img = null)
	...
	str +=
		'<div class="sorting-checkbox"><label class="ba-form-checkbox cancel-sortable">'
	str +=
		'<input type="checkbox" data-ind="' + ind + '"><span></span></label></div>'
...
function setFieldValues(modal) {
	...
	if (app.edit.units) {
		for (let option in app.edit.units) {
			search = modal.find(
				'select.ba-units-select[data-group="units"][data-option="' +
				option +
				'"]'
			)
	...
	if (app.edit.items) {
		modal
			.find('.sorting-group-wrapper .ba-settings-toolbar label.active')
			.removeClass('active')
	...
	for (let ind in app.edit) {
		if (typeof app.edit[ind] != 'object') {
			app.updateEditorInput(
				app.edit[ind],
				modal.find('[data-option="' + ind + '"]').not('[data-group]')
			)
			...
			for (let option in typography) {
				search = modal.find(
					'[data-group="' +
					ind +
					'"][data-subgroup="typography"][data-option="' +
					option +
					'"]'
				)
			...
			for (let option in app.edit[ind].units) {
				search = modal.find(
					'select.ba-units-select[data-group="' +
					ind +
					'"][data-subgroup="units"][data-option="' +
					option +
					'"]'
				)
			...
			for (let option in app.edit[ind]) {
				if (option == 'link') {
					search = modal
						.find('[data-group="' + ind + '"]')
						.not('[data-subgroup]')
					search[app.edit[ind][option] ? 'addClass' : 'removeClass'](
						'link-enabled'
					)
				} else {
					search = modal
						.find('[data-group="' + ind + '"][data-option="' + option + '"]')
						.not('[data-subgroup]')
...
app.showTotalEditor = function () {
	...
	if (!app.edit.promo.expires) {
		...
		str =
			array[2] * 1 +
			' ' +
			formsLanguage[app.shortMonths[array[1] * 1 - 1]] +
			' ' +
			array[0]
	}
	app.toggleCartOptions()
	setFieldValues(modal)
	modal
		.find('.open-calendar-dialog')
		.attr('data-value', app.edit.promo.expires)
		.val(str)
}
...
app.toggleCartOptions = function () {
	if (app.edit.promo.enable) {
		$f(
			'.promo-code-options, .ba-cart-promo-code-wrapper, .ba-cart-discount-row'
		).css('display', '')
	} else {
		$f(
			'.promo-code-options, .ba-cart-promo-code-wrapper, .ba-cart-discount-row'
...
app.showFieldDesign = function () {
	...
	if (app.edit.design) {
		...
		$f('#calculation-field-settings-dialog .field-design-options').css(
			'display',
			''
		)
	}
...
app.closeAdvancedAccordions = function () {
	$f('.advanced-design-settings .accordion-body.in')
		.removeClass('in')
		.css('height', '0px')
		.closest('.accordion-group')
		.removeClass('active')
	...
	if (callback) {
		...
		app.delay = setTimeout(function () {
			if (group == 'form' && subgroup == 'width' && option == 'value') {
				$f('[data-group="form"][data-subgroup="width"][data-option="value"]')
					.not(input)
					.each(function () {
						setLinearWidth(
							$f(this)
				...
				if (option == 'font-size') {
					...
					$f(
						'[data-group="field"][data-subgroup="' +
						subgroup +
						'"][data-option="' +
						option +
						'"]'
					...
					$f(
						'[data-group="label"][data-subgroup="' +
						subgroup +
						'"][data-option="' +
						option +
						'"]'
...
app.executeInputMask = function (input, mask) {
	let n =
		input.beforeData.start +
		Math.abs(input.beforeData.value.length - input.value.length),
		pos = mask.indexOf('_'),
		text = [],
		delData = [],
		...
		value = mask
	if (
		input.value.length > input.beforeData.value.length ||
		input.value.length == input.beforeData.value.length
	) {
		let i = 0
		...
		input.beforeData.start == 0 &&
		input.beforeData.end == mask.length
	) {
		...
		let i =
			input.selectionStart < input.beforeData.start
				? input.selectionStart
				: input.beforeData.start
		n =
			input.selectionStart < input.beforeData.start ? input.beforeData.start : n
		for (i; i < n; i++) {
			delData.push(i)
		}
	}
...
app.setPopularFontsFamily = function () {
	let str = '<li data-value="inherit" class="popular-fonts">Inherit</li>',
		...
		array = new Array(),
		fonts = [
			app.design.label.typography['font-family'],
			app.design.field.typography['font-family']
		]
	for (let i = 0; i < 2; i++) {
		if (
			fonts[i] != 'inherit' &&
			app.design.theme['popular-fonts'].indexOf(fonts[i]) == -1
		) {
	...
	for (let i = 0; i < app.design.theme['popular-fonts'].length; i++) {
		...
		str +=
			'<li data-value="' + value + '" class="popular-fonts">' + text + '</li>'
	}
...
app.setDesignCssVariable = function (
	group,
	subgroup,
	option,
	obj,
	...
	if (group == 'field' && subgroup == 'icon' && option == 'text-align') {
		$f('.ba-forms-workspace-body')
			.removeClass('fields-icons-flex-end fields-icons-flex-start')
			.addClass('fields-icons-' + value)
	}
...
app.setBasicSettings = function () {
	let modal = $f(
		'#design-settings-dialog .basic-design-settings, #custom-color-scheme-dialog'
	)
	for (let subgroup in app.design.theme) {
		if (
			typeof app.design.theme[subgroup] == 'object' &&
			subgroup != 'popular-fonts'
		) {
			for (let option in app.design.theme[subgroup]) {
				let value = app.design.theme[subgroup][option],
					input = modal.find(
						'[data-group="theme"][data-subgroup="' +
						subgroup +
						'"][data-option="' +
						option +
						'"]'
			...
			let value = app.design.theme[subgroup],
				input = modal
					.find('[data-group="theme"][data-option="' + subgroup + '"]')
					.not('[data-subgroup]')
			app.updateEditorInput(value, input)
		}
	}
...
app.updateEditorInput = function (value, input) {
	...
	if (element && element.classList.contains('selected-dates-tags')) {
		...
		for (let key in value) {
			input.append(
				app.getDateTag(key, element.dataset.group, element.dataset.option)
			)
		}
		input
			.closest('.ba-settings-item')
		...
		if (
			element.type == 'textarea' &&
			element.classList.contains('text-editor-textarea')
		) {
		...
		let div = input.closest('.ba-custom-select, .trigger-picker-modal'),
			parent = div.hasClass('trigger-picker-modal')
				? $f('#' + div.attr('data-modal'))
				: div,
...
function showDataTagsDialog(dialog) {
	...
	modal
		...
		.modal()[0]
		.style.setProperty('--picker-arrow-bottom', bottom)
}
...
function installTemplate() {
	if (app.formsTemplates.length > 0) {
		...
		XHR.open(
			'POST',
			'index.php?option=com_baforms&task=form.installTemplate',
			true
		)
...
function installTemplates() {
	...
	str +=
		' <span class="installed-templates-count">0</span> / ' +
		app.formsTemplates.length
	str +=
		'</span><img src="' +
		JUri +
		'components/com_baforms/assets/images/reload.svg"></img>'
...
app.selectIconDialog = function () {
	...
	$f('.select-default-value').on('click', function () {
		fontBtn = this.closest('.ba-settings-item').querySelector(
			'input[type="text"]'
		)
	...
	$f('.select-data-tags-type').on('change', function () {
		...
		modal
			.find(
				'div.ba-settings-group' +
				(this.value ? '.' + this.value + '-data-tags' : '')
			)
			.css('display', '')
	})
	$f('#default-value-dialog')
		.on('show', function () {
			let str = '',
				array = getSubmitItems()
			for (let i = 0; i < array.length; i++) {
				str +=
					'<div class="ba-settings-item ba-settings-input-type"><span class="ba-settings-item-title">'
				if (array[i].title) {
					str += array[i].title
				}
				str += '</span><input type="text" readonly onfocus="this.blur()"'
				str +=
					' class="select-input" value="[Field ID=' + array[i].id + ']"></div>'
			}
			$f(this)
				.find('.fields-data-tags .ba-settings-item')
				.not(':first-child')
				.remove()
			$f(this)
				.find('.fields-data-tags')
				.append(str)
			clearTimeout(this.delay)
		})
		.on('hide', function () {
		...
		.find('.ba-settings-group')
		.on('click', '.ba-settings-input-type', function () {
...
function duplicateField(item, items) {
	...
	if (item.dataset.type == 'checkbox' || item.dataset.type == 'radio') {
		$f(item)
			.find(
				'> .ba-input-wrapper > .ba-field-container input[type="' +
				item.dataset.type +
				'"]'
			)
			.attr('name', item.id)
		...
		$f(item)
			.find('.ba-input-wrapper .ba-field-container input')
			.attr('name', item.id)
	}
}
...
app.hidePreloader = function () {
	$f('.preloader')
		.addClass('ba-hide')
		.removeClass('ba-preloader-slide')
}
...
app.renderFormCalendar = function () {
	let start = 'start' in app.edit ? app.edit.start * 1 : 0,
		...
		daysInMonth =
			32 - new Date(wrapper.dataset.year, wrapper.dataset.month, 32).getDate(),
		today = new Date(),
	...
	for (let i = 0; i < 6; i++) {
		...
		for (let j = start; j <= end; j++) {
			if ((i === 0 && j < firstDay) || date > daysInMonth) {
				...
				if (
					date == today.getDate() &&
					wrapper.dataset.year == today.getFullYear() &&
				...
				let dayStr = (date < 10 ? '0' : '') + date,
					currentTime = +new Date(
						...
						wrapper.dataset.month,
						date
					)
				if (
					fontBtn.dataset.callback == 'setDisableDatesRange' &&
					fontBtn.dataset.index == 1 &&
					fontBtn.rangeBtn[0].dataset.time &&
					currentTime <= fontBtn.rangeBtn[0].dataset.time
				) {
					str += ' disabled-date'
				}
				str +=
	...
	wrapper.querySelector('.ba-calendar-title').textContent =
		formsLanguage[month] + ' ' + wrapper.dataset.year
...
app.getDateTag = function (value, group, option) {
	...
	if (option == 'range-dates') {
		...
		str +=
			date1[2] * 1 +
			' ' +
			formsLanguage[app.shortMonths[date1[1] * 1 - 1]] +
			' ' +
			date1[0] +
			' - '
		str +=
			date2[2] * 1 +
			' ' +
			formsLanguage[app.shortMonths[date2[1] * 1 - 1]] +
			' ' +
			date2[0]
...
app.setDisableDates = function () {
	let wrapper = $f(fontBtn)
		.closest('.ba-settings-item')
		.find('.selected-dates-tags'),
...
app.setCalendarEvents = function () {
	$f('.open-calendar-dialog').on('click', function () {
		var rect = this.getBoundingClientRect(),
			...
			height = modal.innerHeight()
		if (
			this.dataset.callback == 'setDisableDatesRange' &&
			this.dataset.index == 1 &&
			!this.rangeBtn[0].dataset.time
		) {
	...
	$f('.open-calendar-dialog[data-callback="setDisableDatesRange"]').each(
		function () {
			this.rangeBtn = $f(
				'.open-calendar-dialog[data-callback="setDisableDatesRange"]'
			).not(this)
		}
	)
	...
	$f('#calendar-dialog .ba-calendar-body, #disable-week-days-dialog').on(
		'click',
		'div[data-date]',
		function () {
	...
	$f('.selected-dates-tags').on('click', 'i[data-remove]', function () {
		if (this.dataset.group) {
			delete app.edit[this.dataset.group][this.dataset.option][
				this.dataset.remove
			]
		} else {
	...
	$f('.open-disable-days-dialog').on('click', function () {
		...
		fontBtn = this
		modal
			.find('div[data-date="0"]')
		[app.edit.start == 0 ? 'prependTo' : 'appendTo'](wrapper)
...
function prepareItemsText(obj) {
	...
	if (wrapper) {
		text =
			'<span class="ba-form-checkbox-title" contenteditable="true">' +
			obj.title +
			'</span>'
	} else {
		text = obj.title
...
app.checkPageCount = function () {
	...
	pages.each(function (ind) {
		...
		this.querySelector('.ba-form-page-navigation-wrapper').style.setProperty(
			'--progress-navigation-percentage',
			floor + '%'
		)
		this.querySelector('.progress-navigation-percentage').textContent =
			floor + '%'
		...
		str +=
			'<span class="ba-form-page-navigation-title"><span class="ba-form-page-navigation-counter">' +
			(ind + 1) +
			'</span><span class="ba-page-navigation-title" contenteditable="true">' +
			this.dataset.title +
			'</span></span>'
	})
	pages
		.find('.ba-form-page-navigation')
		.html(str)
		.each(function (ind) {
			this.querySelector(
				'.ba-form-page-navigation-title:nth-child(' + (ind + 1) + ')'
...
app.checkFormSlider = function () {
	$f('.form-slider-wrapper input[type="range"]').each(function () {
		if (!this.linear) {
			...
			this.numbers = parent.querySelectorAll(
				'.form-slider-input-wrapper input[data-type="slider"]'
			)
			this.numbers[0].slides = this.slides
			this.numbers[1].slides = this.slides
			this.input = parent.querySelector(
				'.form-slider-input-wrapper input[type="hidden"]'
			)
...
function createNewSelectItem($this, title) {
	let obj = {
	...
	if (parent[0].localName == 'select') {
		...
		option.innerHTML =
			'<div class="ba-checkbox-wrapper"><span class="ba-checkbox-title"></span><label class="ba-form-' +
			item.dataset.type +
			'"><input type="' +
			item.dataset.type +
			'" name="' +
...
app.copyItem = function ($this, items) {
	...
	$f(clone).each(function () {
		...
		setSortable(
			$f(this).find('.ba-page-items'),
			'row',
			'> .ba-form-row > .ba-edit-item .edit-settings',
			'> .ba-form-row'
		)
		setSortable(
			$f(this).find('.ba-form-column'),
			'items',
			handle,
			'> .ba-form-field-item'
		)
	...
	if (!items) {
		...
		$f(clone)
			...
			.each(function () {
				app.createGoogleMap(
					this,
					app.items[this.closest('.ba-form-map-field').id]
				)
			})
		app.buttonsPrevent(
			$f('.ba-forms-workspace').find('a, input[type="submit"], button')
...
app.renderCKE = function () {
	...
	$f('.ba-settings-text-editor-type').on(
		'input',
		'textarea.cke_source',
		function () {
			$f(this)
				.closest('.ba-settings-text-editor-type')
				.find('textarea[data-option]')
				.val(this.value)
				.trigger('input')
		}
	)
...
app.initFormsRecaptcha = function (element) {
	let parent = $f(element)
		.find('.ba-form-submit-recaptcha-wrapper')
		.each(function () {
...
function checkIntegrationsActiveState(ind) {
	...
	if (ind == 'google_maps' || ind == 'telegram') {
		...
		if (ind == 'google_maps' && flag) {
			$f(
				'.ba-form-field[data-key="map"], .ba-form-field[data-key="address"]'
			).removeClass('disabled-field-drag')
		} else if (ind == 'google_maps') {
			$f(
				'.ba-form-field[data-key="map"], .ba-form-field[data-key="address"]'
			).addClass('disabled-field-drag')
		}
	} else if (ind == 'pdf_submissions') {
		flag = obj.enable
		document.body.classList[flag ? 'add' : 'remove'](
			'pdf-submissions-activated'
		)
	...
	if (payments.indexOf(ind) != -1) {
		...
		if (flag) {
			...
			if (ind == 'authorize') {
				...
				title = 'PayPal'
			} else if (ind == 'flutterwave') {
				title = 'Flutterwave'
			}
			parent.append('<option value="' + ind + '">' + title + '</option>')
...
function setIntegrationValues(ind) {
	...
	if (
		ind == 'mailchimp' &&
		obj.api_key &&
		mailchimp[obj.api_key] &&
		obj.list &&
		mailchimp[obj.api_key][obj.list]
	) {
		...
		googleSheets.sheets &&
		obj.spreadsheet &&
		googleSheets.sheets[obj.spreadsheet]
	) {
		drawSpreadsheets(googleSheets)
		if (googleSheets.sheets[obj.spreadsheet].worksheets) {
			let worksheets = googleSheets.sheets[obj.spreadsheet].worksheets
			drawWorksheets(worksheets, obj.spreadsheet)
			if (worksheets[obj.worksheet] && worksheets[obj.worksheet].columns) {
				drawWorkSheetsColumns(worksheets[obj.worksheet].columns)
			}
		}
	} else if (ind == 'activecampaign') {
		$f('.activecampaign-list')
			.closest('.ba-subgroup-element')
		[obj.api_key && obj.account ? 'addClass' : 'removeClass'](
			'visible-subgroup'
		)
		$f('.activecampaign-fields')[
			obj.api_key && obj.account && obj.list ? 'addClass' : 'removeClass'
		]('visible-subgroup')
	} else if (ind == 'getresponse' && obj.api_key && getresponse[obj.api_key]) {
		$f('.getresponse-custom-fields')[
			obj.custom_fields ? 'addClass' : 'removeClass'
		]('visible-subgroup')
	} else if (ind == 'google_drive' && googleDrive.folders) {
		drawDriveFolders(googleDrive)
		$f('.google-drive-fields')[obj.folder ? 'addClass' : 'removeClass'](
			'visible-subgroup'
		)
	}

	if (ind == 'pdf_submissions') {
		$f('.integration-options[data-group="' + ind + '"] .ba-subgroup-element')[
			obj.enable ? 'addClass' : 'removeClass'
		]('visible-subgroup')
	}
	$f('.integration-options[data-group="' + ind + '"] [data-key]').each(
		function () {
			...
			if (trigger) {
				$f('#' + trigger.dataset.modal)
					.find('li[data-value="' + obj[this.dataset.key] + '"]')
					.each(function () {
	...
	if (ready) {
		...
		$f.ajax({
			...
			success: function (msg) {
				...
				for (var ind in obj) {
					str +=
						'<option value="' + ind + '">' + obj[ind].layout_name + '</option>'
				}
				$f('.zoho-crm-list')
					.append(str)
					.closest('.ba-subgroup-element')
					.addClass('visible-subgroup')
			}
		})
	}
}
...
function getZohoCRMFields() {
	$f.ajax({
		...
		success: function (msg) {
			let obj = JSON.parse(msg),
				grant_token = $f('.connect-zoho-crm[data-key="grant_token"]')
					.val()
					.trim(),
				str = ''
			...
			for (var ind in obj) {
				str +=
					...
					ind +
					'"' +
					(integrations.zoho_crm.key.layout_id == ind ? 'selected' : '') +
					'>'
...
function drawZohoCRMFields(fields, checkActive) {
	...
	for (let i in fields) {
		str +=
			...
			(fields[i].req ? ' data-required="true"' : '') +
			'><span class="ba-settings-item-title">' +
			fields[i].label +
			(fields[i].req ? ' *' : '') +
			'</span><select class="forms-fields-list" data-key="' +
			fields[i].dv +
			'"><option value="" hidden></option></select></div>'
	}
...
function connectActivecampaign(account, api_key, notice, checkActive) {
	$f.ajax({
		...
		dataType: 'text',
		url:
			'index.php?option=com_baforms&view=form&task=form.getActivecampaignLists',
		data: {
			api_key: api_key,
			account: account
		},
		success: function (msg) {
			...
			if (obj.success) {
				...
				$f(
					'.integration-options[data-group="activecampaign"] .ba-subgroup-element'
				).removeClass('visible-subgroup')
			}
		}
	})
}
	...
	$f.ajax({
		...
		success: function (msg) {
			...
			if (obj.success) {
				...
				$f('.campaign-monitor-list')
					.append(str)
					.closest('.ba-subgroup-element')
					.addClass('visible-subgroup')
				if (checkActive && integrations.campaign_monitor.key.list_id) {
					getCampaignMonitorFields(
						api_key,
						client_id,
						integrations.campaign_monitor.key.list_id,
						true
					)
				}
			} else {
				...
				$f(
					'.integration-options[data-group="campaign_monitor"] .ba-subgroup-element'
				).removeClass('visible-subgroup')
			}
		}
	})
...
function drawCampaignMonitorFields(fields, checkActive) {
	...
	for (let i in fields) {
		str +=
			'<div class="ba-settings-item ba-settings-select-type"><span class="ba-settings-item-title">' +
			fields[i].title +
			'</span><select class="forms-fields-list" data-key="' +
			fields[i].key +
			'"><option value="" hidden></option></select></div>'
		...
		success: function (msg) {
			...
			if (obj.success) {
				...
				if (checkActive && integrations.getresponse.key.list_id) {
					getResponseFields(
						api_key,
						integrations.getresponse.key.list_id,
						custom,
						true
					)
				}
				...
				$f(
					'.integration-options[data-group="getresponse"] .ba-subgroup-element'
				).removeClass('visible-subgroup')
			}
		}
	})
...
function drawGetResponseFields(fields, custom, checkActive) {
	let div = $f('.getresponse-fields'),
		...
		customDiv = div
			.find('.getresponse-custom-fields')
		[custom ? 'addClass' : 'removeClass']('visible-subgroup'),
		str = ''
	...
	$f.ajax({
		...
		success: function (msg) {
			if (msg == 0) {
				...
				$f(
					'.integration-options[data-group="mailchimp"] .ba-subgroup-element'
				).removeClass('visible-subgroup')
			} else {
				let obj = JSON.parse(msg),
					str = ''
				if (obj.lists) {
					...
					if (checkActive && integrations.mailchimp.key.list) {
						getMailChimpFields(
							integrations.mailchimp.key.api_key,
							integrations.mailchimp.key.list,
							true
						)
					}
				} else if (notice) {
...
function drawSpreadsheets(obj) {
	...
	for (let ind in obj.sheets) {
		select.append(
			...
			obj.sheets[ind].id +
			'">' +
			obj.sheets[ind].title +
			'</option>'
		)
	}
}

function drawWorksheets(obj, spreadsheet) {
	...
	for (let ind in obj) {
		select.append(
			'<option value="' + obj[ind].id + '">' + obj[ind].title + '</option>'
		)
	}
}

function drawWorkSheetsColumns(columns) {
...
function getWorkSheets(accessToken, spreadsheet, obj, flag, worksheet) {
	let object = {
	...
	$f.ajax({
		...
		success: function (msg) {
			obj.sheets[spreadsheet].worksheets = JSON.parse(msg)
			if (flag) {
				notification[0].className = 'animation-out'
				document.querySelector(
					'.google-sheet-spreadsheets'
				).dataset.authentication = 'enabled'
			}
			drawWorksheets(obj.sheets[spreadsheet].worksheets, spreadsheet)
			if (worksheet) {
				getWorkSheetsColumns(accessToken, spreadsheet, obj, flag, worksheet)
			}
		}
	})
}

function getWorkSheetsColumns(accessToken, spreadsheet, obj, flag, worksheet) {
	let object = {
		id: obj.sheets[spreadsheet].id,
		title: obj.sheets[spreadsheet].title
	}
	$f.ajax({
		...
		dataType: 'text',
		url:
			'index.php?option=com_baforms&view=form&task=form.getWorkSheetsColumns',
		data: {
		...
		success: function (msg) {
			obj.sheets[spreadsheet].worksheets[worksheet].columns = JSON.parse(msg)
			if (flag) {
				document.querySelector(
			...
			drawWorkSheetsColumns(
				obj.sheets[spreadsheet].worksheets[worksheet].columns
			)
		}
	})
}
...
app.checkLastCheckbox = function (id, count) {
	...
	wrapper
		.find('.last-row-checkbox-wrapper')
		.removeClass('last-row-checkbox-wrapper')
	wrapper
		.find('.ba-form-checkbox-wrapper')
		.each(function () {
			if (ind == count) {
		...
		.last()
		.addClass('last-row-checkbox-wrapper')
}
	...
	app.conditionLogic.forEach(function (el) {
		...
		el.when.forEach(function (obj) {
			...
			if (item.dataset.type == 'radio' || item.dataset.type == 'select') {
				item
					.querySelectorAll('option:not([hidden]), .ba-checkbox-wrapper input')
				...
				item.dataset.type == 'selectMultiple'
			) {
				value = []
				item
					.querySelectorAll('option:not([hidden]), .ba-checkbox-wrapper input')
				...
				value = checked
					? item.querySelector('.ba-form-acceptance-html').textContent.trim()
					: ''
			} else {
			...
			switch (obj.state) {
				...
				case 'greater':
					flags.push(
						(!isNaN(value) ? value * 1 : value) >
						(!isNaN(obj.value) ? obj.value * 1 : obj.value)
					)
					break
				case 'less':
					flags.push(
		...
		let method = el.operation == 'OR' ? 'some' : 'every',
			flag = flags[method](function (state) {
				return state
			})
...
app.setSubmitDesign = function (obj, wrapper) {
	...
	for (let i = 0; i < keys.length; i++) {
		for (let option in obj[keys[i]]) {
			...
			if (
				(keys[i] == 'padding' && (option == 'top' || option == 'bottom')) ||
...
app.prepareFormsPages = function () {
	...
	$f('.ba-form-submit-field').each(function () {
		app.setSubmitDesign(
			app.items[this.id],
			this.querySelector('.ba-form-submit-wrapper')
		)
	...
	$f('.page-info').text(
		app.design.form.width.value + app.design.form.units['width-value']
	)
	app.checkFormSlider()
...
document.addEventListener('DOMContentLoaded', function () {
	...
	app.items.navigation = $f.extend(
		true,
		formOptions.navigation,
	...
	setSortable(
		$f('.ba-forms-workspace-body'),
		'page',
		'> .ba-form-page > .ba-edit-item .edit-settings',
		'> .ba-form-page',
		function () {
			app.checkPageCount()
		}
	)
	$f('.ba-forms-workspace-body').on(
		'mousedown',
		'.ba-column-resizer',
		columnResizer
	)
	...
	$f('.edit-lightbox-color')
		.on('mousedown', function (event) {
			var rect = this.getBoundingClientRect(),
				rect2 = document
					.querySelector('.edit-lightbox')
			...
			modal
				.removeClass('ba-right-position ba-bottom-position')
				.addClass('ba-top-position')
			setMinicolorsColor(this.dataset.rgba)
			fontBtn = this
			setTimeout(function () {
				modal
	...
	$f('#apply-delete').on('click', function () {
		if (
			typeof fontBtn == 'object' &&
			fontBtn.type == 'deleteSelectedCondition'
		) {
			let array = [],
				ind = 0
			fontBtn.rows.each(function (i) {
			...
			typeof fontBtn == 'object' &&
			fontBtn.type == 'deleteConditionRow'
		) {
			let array = [],
				key = fontBtn.row.dataset.ind,
				ind = 0
			$f(fontBtn.parent)
				.find('.condition-logic-horizontal-fields-wrapper')
				.each(function (i) {
					if (fontBtn.row == this) {
						this.remove()
					} else {
			...
			app.edit[key] = array
			app.checkConditionLogic()
			$f('.conditions-matches-wrapper').css(
				'display',
				app.edit.when.length > 1 ? '' : 'none'
			)
		} else if (
			typeof fontBtn == 'object' &&
			fontBtn.type == 'deleteSelectedItems'
		) {
			let object = {},
				item = document.querySelector('#' + app.selector),
				ind = 0
			...
			if (
				item &&
				item.dataset.type == 'total' &&
				!item.querySelector('.ba-cart-shipping-item')
			) {
			...
			$f('.sorting-group-wrapper label[data-action].active').removeClass(
				'active'
			)
			if (app.edit.type == 'navigation') {
				app.checkPageCount()
			...
			typeof fontBtn == 'object' &&
			fontBtn.type == 'deleteSelectedItemsImage'
		) {
			...
			$f(
				'#' +
				app.selector +
				' .ba-form-checkbox-wrapper:nth-child(' +
				(ind + 1) +
				') .ba-checkbox-image'
			).remove()
			app.checkLastCheckbox(app.selector, app.edit.count)
	...
	$f('body').on('mousedown', function (event) {
		if (
			...
			!(
				...
				((event.target.className &&
					...
					$f(event.target).closest('[class^="mce"], [class^="cke_"]').length >
					0)
			)
		) {
			$f('.modal-backdrop')
		...
		if (
			event.target &&
			(event.target.classList.contains('close-all-modals') ||
				event.target.closest('.close-all-modals'))
		) {
	...
	$f('body .modal')
		...
		.on('shown', function (event) {
			$f('.modal-backdrop')
				...
				.addClass(
					this.id +
					(this.classList.contains('hidden-modal-backdrop')
						? ' hidden-modal-backdrop'
						: '')
				)
		})
	$f('.ba-modal-cp > *, #text-editor-dialog > *').on('mousedown', function (
		event
	) {
		let $this = this.closest('.modal')
		if (
			$f('.ba-modal-picker, .picker-modal-arrow')
				.not($this)
				.hasClass('in') &&
			event.target != fontBtn
		) {
			$f('.modal-backdrop.in')
				.last()
				.trigger('click')
	...
	$f('body').on('click', '.ba-add-rows', function () {
		...
		this.closest('.ba-form-page')
			.querySelector('.ba-page-items')
			.appendChild(content)
	})
	$f('body').on('click', '.modify-columns', function () {
		let rect = this.closest('.ba-edit-item')
			.querySelector('.edit-settings')
			.getBoundingClientRect(),
			row = this.closest('.ba-form-row'),
			columns = $f(row).find('> .ba-form-column-wrapper > .ba-form-column')
				.length,
	...
	$f('.design-settings-switcher, .hide-design-settings').on(
		'mousedown',
		function () {
			document
				.querySelector('.advanced-design-settings')
				.classList[this.dataset.action]('active')
	...
	$f('.color-scheme-item').on('click', function () {
		if (this.dataset.scheme == 'custom') {
			...
			$f('#custom-color-scheme-dialog input[data-type="color"]').each(
				function () {
					let input = $f(this)
	...
	$f('.fonts-select').on('change', function () {
		...
		if (input.dataset.group || input.dataset.subgroup || input.dataset.option) {
			...
			if (input.dataset.group == 'theme') {
				...
				$f('[data-subgroup="typography"][data-option="font-family"]').each(
					function () {
						this.value = value
						$f(this)
							.prev()
							.val(googleFonts[value])
					}
				)
	...
	$f('.ba-settings-toolbar label[data-option]').on('click', function () {
		...
		if (this.dataset.default) {
			...
			parent
				.querySelector(
					'label[data-option="' + this.dataset.option + '"].active'
				)
				.classList.remove('active')
			classList.add('active')
		}
	...
	$f('i.spacing-link').on('click', function () {
		let flag = this.classList.contains('link-enabled'),
			str =
				...
				'"]' +
				(this.dataset.subgroup
					? '[data-subgroup="' + this.dataset.subgroup + '"]'
					: '')
		...
		if (flag) {
			$f(this)
				.closest('.ba-settings-item')
				.find(str)
				.removeClass('link-enabled')
		} else {
			$f(this)
				.closest('.ba-settings-item')
				.find(str)
				.addClass('link-enabled')
		}
	})
	$f('select.ba-units-select').on('change', function () {
		let str =
			...
			'"]',
			select = $f(this)
				.closest('#design-settings-dialog')
				.find('.ba-units-select' + str),
		...
		if (this.dataset.option == 'letter-spacing') {
			...
			this.value == 'px' &&
			this.dataset.option != 'width-value' &&
			this.dataset.option != 'width'
		) {
			input.val(14)
		} else {
	...
	$f('.ba-form-layout-select').on('change', function () {
		...
		$f('.ba-forms-workspace')[
			this.value == 'lightbox' ? 'addClass' : 'removeClass'
		]('ba-forms-lightbox-enabled')
	})
	var str =
		'.ba-settings-number-type, .ba-settings-input-type, .ba-settings-textarea-type, .ba-settings-text-editor-type'
	$f(str)
		.find('input[data-option], textarea[data-option]')
		.on('input', function () {
			let $this = this
			clearTimeout(this.delay)
			this.delay = setTimeout(function () {
	...
	$f('.ba-settings-font-type input[data-option][data-callback]').on(
		'change',
		function () {
			app.setValue(
				this.value,
	...
	str =
		'.ba-input-label-wrapper, .ba-form-checkbox-title, .ba-page-navigation-title'
	$f('.ba-forms-workspace-body')
		.on('keydown', str, function (event) {
			if (event.keyCode == 13) {
				event.preventDefault()
				event.stopPropagation()
			}
		...
		.on('click', '.ba-phone-selected-country', function () {
			...
			setTimeout(function () {
				...
				$f('body')
					.off('click.selected-country')
					.one('click.selected-country', function () {
						$f('.visible-countries-list').removeClass('visible-countries-list')
					})
			}, 100)
		})
		.on('input', '.ba-phone-countries-search', function () {
			var li = this.closest('.ba-phone-countries-wrapper').querySelectorAll(
				'li'
			),
				search = this.value.toLowerCase()
			clearTimeout(this.delay)
			this.delay = setTimeout(function () {
				li.forEach(function (el) {
					...
					el.style.display =
						!search || title.indexOf(search) != -1 ? '' : 'none'
				})
			}, 300)
		})
		...
		.on(
			'click',
			'.ba-phone-countries-list li.ba-phone-country-item',
			function () {
				let wrapper = this.closest('.ba-field-container')
				if (!wrapper.phoneInput) {
					wrapper.countryFlag = wrapper.querySelector(
						'.ba-phone-selected-country .ba-phone-flag'
					)
					wrapper.countryPrefix = wrapper.querySelector(
						'.ba-phone-selected-country .ba-phone-prefix'
					)
					wrapper.phoneInput = wrapper.querySelector(
						'input.ba-phone-number-input'
					)
				}
				wrapper.countryFlag.className =
					'ba-phone-flag ba-phone-flag-' + this.dataset.flag
		...
		.on(
			'input',
			'.ba-form-phone-field input.ba-phone-number-input',
			function () {
		...
		.on('keyup', '.ba-page-navigation-title', function (event) {
			...
			this.delay = setTimeout(function () {
				let nth =
					$this
						.closest('.ba-form-page-navigation-title')
						.querySelector('.ba-form-page-navigation-counter').textContent * 1
				app.items.navigation.items[nth - 1].title = $this.textContent
				document.querySelector(
					'.ba-form-page:nth-child(' + nth + ')'
				).dataset.title = $this.textContent
				$f(
					'.ba-form-page-navigation .ba-form-page-navigation-title:nth-child(' +
					nth +
					')'
				)
					.find('.ba-page-navigation-title')
					.not($this)
		...
		.on('keyup', '.ba-form-checkbox-title', function () {
			...
			this.delay = setTimeout(function () {
				$f($this)
					.closest('.ba-form-checkbox-group-wrapper')
					.find('> .ba-form-checkbox-wrapper .ba-form-checkbox-title')
					.each(function (ind) {
						if (this == $this) {
		...
		.on(
			'change',
			'.ba-form-acceptance-field input[type="checkbox"]',
			function () {
				app.checkConditionLogic()
			}
		)
		.on('change', '.ba-form-checkbox-group-wrapper', function () {
			...
			if (image) {
				$f(this)
					.find('.checked-image-container')
					.removeClass('checked-image-container')
			}
			for (let i = 0; i < input.length; i++) {
				if (input[i].checked) {
					input[i]
						.closest('.ba-form-checkbox-wrapper')
						.classList.add('checked-image-container')
				}
			}
			app.checkConditionLogic()
		})
		.on('click', '.ba-form-checkbox-wrapper', function (event) {
			if (
				$f(this).find('> .ba-checkbox-image').length &&
				!event.target.classList.contains('ba-form-checkbox') &&
				!event.target.closest('.ba-form-checkbox') &&
				!event.target.closest('.ba-form-radio')
			) {
				$f(this)
					.find('> .ba-checkbox-wrapper input')
					.each(function () {
		...
		.on('input', '.form-range-wrapper input[type="range"]', function () {
			var max = this.max * 1,
				min = this.min * 1
				; (value = this.value * 1), (sx = ((value - min) * 100) / (max - min))
			if (!this.linear) {
				this.linear = this.closest('.form-range-wrapper').querySelector(
					'.ba-form-range-liner'
				)
			}
			if (!this.number) {
				this.number = this.closest('.ba-field-container').querySelector(
					'.form-slider-input-wrapper input[data-type="range"]'
				)
			}
		...
		.on(
			...
			function () {
				if (!this.range) {
					this.range = $f(this)
						.closest('.ba-field-container')
						.find('.form-range-wrapper input[type="range"]')
				}
		...
		.on('input', '.form-slider-wrapper input[type="range"]', function () {
			...
			sx =
				((this.input.values[1] * 1 - this.input.values[0] * 1) * 100) /
				(max - min)
	...
	$f('.modify-item-suffix').on('input', function () {
		...
		this.delay = setTimeout(function () {
			if (app.edit.theme) {
				$f('.ba-forms-workspace-body')
					.removeClass(app.edit.theme.suffix)
					.addClass($this.value)
			} else if (app.edit.session) {
				$f('.ba-forms-workspace-body')
					.removeClass(app.edit.suffix)
					.addClass($this.value)
				...
				$f('.ba-form-page-navigation-wrapper, .ba-form-page-break')
					.removeClass(app.edit.suffix)
					.addClass($this.value)
			}
			if (app.edit.theme) {
	...
	$f('.copy-to-clipboard').on('click', function (event) {
		...
		textarea.value = this.closest('.ba-settings-input-type').querySelector(
			'.select-input'
		).value
		textarea.select()
	...
	$f('.ba-settings-sortable-type .sorting-container').on(
		'input',
		'input[data-key]',
		function () {
			...
			this.delay = setTimeout(function () {
				if ('promo' in app.edit && key == 'title') {
					$f('#' + app.selector)
						.find('.ba-form-shipping-title')
						.get(ind).textContent = app.edit.items[ind].title
				} else if ('promo' in app.edit && key == 'price') {
					let price = renderPrice(
						app.edit.items[ind].price,
						app.edit.thousand,
						app.edit.separator,
						app.edit.decimals
					),
						option = $f('#' + app.selector)
							.find('.ba-cart-shipping-item')
							.get(ind)
					option.querySelector('.field-price-value').textContent = price
					option.querySelector('input[type="radio"]').dataset.price =
						app.edit.items[ind].price
				} else if (app.edit.type != 'navigation') {
					let option = $f('#' + app.selector)
						.find('select option:not([hidden]), span.ba-checkbox-title')
						.get(ind),
						action = option.localName == 'option' ? 'textContent' : 'innerHTML'
					option[action] = prepareItemsText(app.edit.items[ind])
				} else {
					$f('.ba-form-page:nth-child(' + (ind + 1) + ')').attr(
						'data-title',
						app.edit.items[ind].title
					)
	...
	$f('.sorting-group-wrapper').on('change', '.ba-form-checkbox', function () {
		let checked = 0
		$f(this)
			.closest('.sorting-container')
			.find('.ba-form-checkbox input')
			.each(function () {
				if (this.checked) {
					checked++
				}
		...
		if (checked) {
			$f(this)
				.closest('.sorting-group-wrapper')
				.find('.ba-settings-toolbar label')
				.addClass('active')
		} else {
			$f(this)
				.closest('.sorting-group-wrapper')
		...
		if (checked == 1) {
			$f(this)
				.closest('.sorting-group-wrapper')
				.find('.ba-settings-toolbar')
				.find('label[data-action="default"], label[data-action="image"]')
				.addClass('active')
		} else {
			$f(this)
				.closest('.sorting-group-wrapper')
	...
	$f('.sorting-group-wrapper').on(
		'click',
		'.delete-sorting-image',
		function () {
			fontBtn = {
	...
	$f('.sorting-group-wrapper .ba-settings-toolbar label').on(
		'click',
		function () {
			if (this.classList.contains('active')) {
				let keys = new Array(),
					rows = $f(this)
						.closest('.sorting-group-wrapper')
						.find('.sorting-container .sorting-item'),
					item =
						app.edit.type != 'navigation'
							? $f('#' + app.selector)
							: $f('.ba-form-page')
				option =
					app.edit.type != 'navigation'
						? item.find(
							'select option:not([hidden]), div.ba-form-checkbox-wrapper'
						)
						: item
				...
				$f(this)
					.closest('.sorting-group-wrapper')
					.find('.sorting-container .ba-form-checkbox input')
					.each(function () {
						if (this.checked) {
					...
					case 'copy':
						...
						option.each(function (i) {
							...
							if (keys.indexOf(i) != -1) {
								...
								if (app.edit.type == 'navigation') {
									clone.querySelector('.sorting-title input').value =
										object[ind].title
								}
								$f($this).after(clone)
								ind++
	...
	$f('.apply-bulk-items').on('click', function () {
		...
		if (textarea.val().trim()) {
			...
			data.forEach(function (title) {
				if (title.trim()) {
					createNewSelectItem(fontBtn, title)
				}
			})
		}
		modal.modal('hide')
	})
	$f('.sorting-group-wrapper .add-new-select-item').on('click', function (
		event
	) {
		if ('promo' in app.edit) {
			let obj = {
				...
				item = document.querySelector('#' + app.selector),
				parent = $f(item)
					.find('.ba-cart-shipping-row')
					.css('display', '')
					.find('.ba-cart-row-content'),
				wrapper = this.closest('.sorting-group-wrapper'),
				row = wrapper.querySelector('.sorting-item:last-child'),
				ind = row ? row.dataset.ind * 1 + 1 : 0,
				option = document.createElement('div'),
			...
			option.innerHTML =
				'<label class="ba-form-radio"><input type="radio" data-price="0" name="shipping-' +
				item.id +
				'"><span></span></label><span class="ba-shipping-title"><span class="ba-form-shipping-title">' +
				obj.title +
				'</span></span><div class="ba-form-calculation-price-wrapper"><span class="field-price-currency">' +
				app.edit.symbol +
				'</span><span class="field-price-value">' +
				price +
				'</span></div>'
			app.edit.items[ind] = obj
			wrapper.querySelector('.sorting-container').appendChild(child)
	...
	setSortable(
		$f('.sorting-group-wrapper .sorting-container'),
		'select',
		'.sortable-handle',
		'> .sorting-item',
		function (el) {
			let object = {},
				...
				wrapper =
					app.edit.type != 'navigation'
						? item.find('select, .ba-form-checkbox-group-wrapper')
						: item,
				options =
					app.edit.type != 'navigation'
						? wrapper.find('option:not([hidden]), .ba-form-checkbox-wrapper')
						: null,
				option = null
			if (app.edit.type == 'navigation') {
	...
	$f('#jform_recaptcha option').each(function () {
		let select = $f(this)
			.closest('.ba-settings-select-type')
			.find('select[data-option="recaptcha"]')
	...
	$f('.add-notifications-admin-emails').on('keyup', function (event) {
		if (
			...
			$f(this)
				.closest('.selected-dates-wrapper')
				.find('.selected-dates-tags')
				.append(str)
			this.value = ''
		}
	})
	$f('#google-maps-field-settings-dialog')
		.on('shown', function () {
			app.createLocationMap()
		})
		.on('hide', function () {
	...
	for (let ind in integrations) {
		...
		if (ind == 'mailchimp' && integrations.mailchimp.key.api_key) {
			...
			connectCampaignMonitor(
				integrations.campaign_monitor.key.api_key,
				integrations.campaign_monitor.key.client_id,
				false,
				true
			)
			...
			connectActivecampaign(
				integrations.activecampaign.key.account,
				integrations.activecampaign.key.api_key,
				false,
				true
			)
		} else if (ind == 'getresponse' && integrations.getresponse.key.api_key) {
			connectGetResponse(
				integrations.getresponse.key.api_key,
				integrations.getresponse.key.custom_fields,
				false,
				true
			)
	...
	$f('.apply-integration-element').on('click', function (event) {
		...
		if (group == 'mailchimp' || group == 'campaign_monitor') {
			obj.key = {}
		}
		...
		$f.ajax({
			type: 'POST',
			dataType: 'text',
			url: 'index.php?option=com_baforms&task=form.saveIntegration',
			data: data,
			complete: function (msg) {
	...
	$f('.mailchimp-list').on('change', function () {
		let api_key = $f('.mailchimp-api-key')
			.val()
			.trim(),
			list_id = $f(this).val()
	...
	if (integrations.google_drive.key.accessToken) {
		$f.ajax({
			type: 'POST',
			dataType: 'text',
			url: 'index.php?option=com_baforms&view=form&task=form.getDriveFolders',
			data: {
			...
			success: function (msg) {
				...
				googleDrive.folders = JSON.parse(msg)
				drawDriveFolders(googleDrive)
	...
	$f('.authenticate-google-drive').on('input', function () {
		if (this.dataset.authentication == 'pending' && this.value.trim()) {
			...
			app.googleDrive = {}
			let parent = $f('.integration-options[data-group="google_drive"]')
			parent.find('select').val('')
			parent
				.find('.ba-subgroup-element.visible-subgroup')
				.removeClass('visible-subgroup')
			return false
		}
		let $this = this
		...
		this.delay = setTimeout(function () {
			...
			$f.ajax({
				...
				dataType: 'text',
				url:
					'index.php?option=com_baforms&view=form&task=form.createDriveToken',
				data: {
				...
				success: function (msg) {
					...
					if (!msg || msg == 'INVALID_TOKEN') {
						showNotice('Invalid Authentication Code', 'ba-alert')
					} else {
						app.googleDrive = JSON.parse(msg)
						notification[0].className = 'animation-out'
						drawDriveFolders(app.googleDrive)
					}
				}
			})
		}, 500)
	...
	if (integrations.google_sheets.key.accessToken) {
		$f.ajax({
			type: 'POST',
			dataType: 'text',
			url: 'index.php?option=com_baforms&view=form&task=form.getSpreadSheets',
			data: {
			...
			success: function (msg) {
				...
				googleSheets.sheets = JSON.parse(msg)
				drawSpreadsheets(googleSheets)
	...
	$f('.authenticate-google-sheets').on('input', function () {
		if (this.dataset.authentication == 'pending' && this.value.trim()) {
			...
			app.googleSheets = {}
			let parent = $f('.integration-options[data-group="google_sheets"]')
		...
		this.delay = setTimeout(function () {
			...
			$f.ajax({
				...
				dataType: 'text',
				url:
					'index.php?option=com_baforms&view=form&task=form.createSheetsToken',
				data: {
					code: $this.value.trim()
				},
				success: function (msg) {
					...
					if (!msg || msg == 'SHEETS_INVALID_TOKEN') {
						...
						app.googleSheets = JSON.parse(msg)
						notification[0].className = 'animation-out'
						drawSpreadsheets(app.googleSheets)
					}
				}
			})
		}, 500)
	...
	$f('.google-sheet-spreadsheets').on('change', function () {
		...
		if (this.dataset.authentication == 'pending') {
			...
			drawWorksheets(app.googleSheets.sheets[value].worksheets, value)
		}
	...
	$f('.google-sheet-worksheets').on('change', function () {
		...
		if (this.dataset.authentication == 'pending') {
			...
			app.googleSheets.sheets[this.spreadsheet].worksheets[value].columns
		) {
			drawWorkSheetsColumns(
				app.googleSheets.sheets[this.spreadsheet].worksheets[value].columns
			)
			...
			if (event.target.localName == 'input') {
				...
				this.delay = setTimeout(function () {
					if (obj.api_key.value.trim() && obj.account.value.trim()) {
						connectActivecampaign(
							obj.account.value.trim(),
							obj.api_key.value.trim(),
							true
						)
					}
				}, 500)
	...
	$f('.connect-getresponse')
		...
		.on('input', function (event) {
			...
			if (event.target.localName == 'input' && event.target.type == 'text') {
				...
				$f('.getresponse-custom-fields')[
					obj.custom_fields.checked ? 'addClass' : 'removeClass'
				]('visible-subgroup')
			}
		})
	...
	$f('.zoho-crm-list').on('change', function () {
		drawZohoCRMFields(
			zoho_crm[
				document.querySelector('.connect-zoho-crm[data-key="grant_token"]')
					.value
			][this.value].fields
		)
	})

	$f('.integration-options[data-group="pdf_submissions"] input[data-key="enable"]'
	).on('change', function () {
		$f(this)
			.closest('.integration-options')
			.find('.ba-subgroup-element')
		[this.checked ? 'addClass' : 'removeClass']('visible-subgroup')
	})
	...
	$f('#condition-logic-modal').on('show', function () {
		...
		modal
			.find('label[data-action="publish"] span[data-action]')
			.each(function () {
				this.style.display = this.dataset.action == 'unpublish' ? '' : 'none'
			})
		modal.find('label[data-action="publish"] i')[0].className =
			'zmdi zmdi-eye-off'
		app.conditionLogic.forEach(function (el, i) {
			...
			li.querySelector('.conditional-logic-icons').innerHTML = el.publish
				? ''
	...
	$f('#condition-logic-modal .add-new-condition-logic').on(
		'click',
		function () {
			if (!app.state) {
				app.showLogin(null)
	...
	$f('#condition-logic-modal ul')
		.on('input', '.conditional-logic-title input', function (event) {
			let $this = this
			clearTimeout(this.delay)
			this.delay = setTimeout(function () {
				app.conditionLogic[
					$this.closest('li').dataset.key
		...
		.on('change', 'input[type="checkbox"]', function () {
			...
			if (array.length == 1) {
				let publish =
					app.conditionLogic[array[0].closest('li').dataset.key].publish
				$f('#condition-logic-modal label[data-action="publish"]')
					.addClass('active')
					.each(function () {
						this.querySelector('i').className = publish
							? 'zmdi zmdi-eye-off'
							: 'zmdi zmdi-eye'
						this.querySelector(
							'span[data-action="publish"]'
						).style.display = publish ? 'none' : ''
						this.querySelector(
				...
				$f('#condition-logic-modal label[data-action="publish"]')
					...
					.each(function () {
						...
						this.querySelector('span[data-action="publish"]').style.display =
							'none'
		...
		.on('click', 'li', function () {
			...
			this.classList.add('active')
			changeSelected(
				modal.find('.conditions-matches-operation'),
				app.edit.operation
			)
			$f('.conditions-matches-wrapper').css(
				'display',
	...
	$f('#condition-logic-modal label[data-action]').on('click', function () {
		if (this.classList.contains('active')) {
			...
			$f('#condition-logic-modal ul li input[type="checkbox"]').each(
				function () {
					if (this.checked) {
			...
			switch (this.dataset.action) {
				case 'publish':
					...
					rows[keys[0]].querySelector(
						'.conditional-logic-icons'
					).innerHTML = publish ? '' : '<i class="zmdi zmdi-eye-off"></i>'
					this.querySelector('i').className = publish
						? 'zmdi zmdi-eye-off'
						: 'zmdi zmdi-eye'
					this.querySelector(
	...
	$f('#condition-logic-modal .ba-settings-group')
		.on('change input', '.condition-logic-horizontal-fields-wrapper', function (
			event
		) {
			let $this = this
			clearTimeout(this.delay)
			this.delay = setTimeout(function () {
				app.edit[$this.dataset.ind][$this.dataset.key][
					event.target.dataset.key
				] = event.target.value
				if (
					$this.dataset.ind == 'when' &&
					event.target.dataset.key == 'field'
				) {
					...
					$this.dataset.ind == 'do' &&
					event.target.dataset.key == 'action'
				) {
	...
	$f('#templates-modal .integrations-group').on(
		'click',
		'.templates-element',
		function (event) {
			event.preventDefault()
					...
					success: function (msg) {
						try {
							...
							pages = Array.prototype.slice.call(
								div.querySelectorAll('.ba-form-page')
							)
							pages.forEach(function (page) {
								let $this = page.querySelector(
									'.ba-form-page > .ba-edit-item .copy-item'
								),
								...
								pageFields.forEach(function (field, i) {
									data.fields[field.id.replace('baform-', '')] = cloneFields[
										i
									].id.replace('baform-', '')
								})
								clones.push(clone)
							...
							clones.forEach(function (clone) {
								clone
									.querySelectorAll('.ba-form-calculation-field')
	...
	$f('#login-modal')
		.on('show', function () {
			var url =
				'https://www.balbooa.com/demo/index.php?option=com_baupdater&view=baforms',
				domain = window.location.host.replace('www.', '')
	...
	$f('.lightbox-position-wrapper div').on('click', function () {
		...
		this.closest('.lightbox-position-wrapper')
	...
	$f('.ba-modal-list-picker').on('click', 'li[data-value]', function () {
		$f(this)
			.closest('.modal')
			.modal('hide')
		fontBtn.querySelector('input[type="text"]').value = this.textContent
		$f(fontBtn)
			.find('input[type="hidden"]')
			.val(this.dataset.value)
			.trigger('change')
	...
	setSortable(
		$f('.ba-form-column'),
		'items',
		'> .ba-form-field-item > .ba-edit-item .edit-settings',
		'> .ba-form-field-item'
	)
	app.prepareFormsPages()
	app.prepareNumbers()
...
app.createTemplateData = function () {
	...
	var btn =
		'<span class="open-create-template" onclick="$f(\'#create-template-modal\').modal();">' +
		'<a href="#"><span class="zmdi zmdi-assignment"></span></a>' +
		'<span class="ba-tooltip ba-right ba-hide-element">Create Template</span>' +
		'</span>',
		html =
			'<div id="create-template-modal" class="mce-ba-modal ba-modal-sm modal hide" style="display:none;">' +
...
!(function ($) {
	...
	gridSorting.prototype = {
		...
		init: function () {
			let $this = this
			this.item.on('mousedown.gridSorting', this.options.handle, function (
				event
			) {
				if (event.button == 0) {
					...
					document.body.classList.add(
						$this.options.group + '-grid-sorting-started'
					)
					$(document)
						.on('mousemove.gridSorting', function (event) {
							let target = null,
							...
							if (
								elements.right &&
								right > elements.right.rect.left + elements.right.rect.width / 2
							) {
								...
								elements.bottom &&
								bottom >
								elements.bottom.rect.top + elements.bottom.rect.height / 2
							) {
								target = elements.bottom.el
								place.insertAfter(target)
							} else if (
								elements.left &&
								left < elements.left.rect.left + elements.left.rect.width / 2
							) {
								target = elements.left.el
								place.insertBefore(target)
							} else if (
								elements.top &&
								top < elements.top.rect.top + elements.top.rect.height / 2
							) {
						...
						.off('mouseleave.gridSorting')
						.on('mouseleave.gridSorting', function () {
							$(document).trigger('mouseup.gridSorting')
						})
						.off('mouseup.gridSorting')
						.on('mouseup.gridSorting', function () {
							...
							$(document).off(
								'mousemove.gridSorting mouseup.gridSorting mouseleave.gridSorting'
							)
						})
